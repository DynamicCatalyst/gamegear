...
public class CartController {
    ...
    public ResponseEntity<ApiResponse> getUserCart(@PathVariable Long userId){
        ...
        if (cart == null) {
            // User has no cart yet (hasn't added anything). Return an empty cart
            // instead of letting ModelMapper throw "source cannot be null".
            CartDto emptyCart = new CartDto();
            emptyCart.setItems(java.util.Set.of());
            emptyCart.setTotalAmount(java.math.BigDecimal.ZERO);
            return ResponseEntity.ok(new ApiResponse("Success", emptyCart));
        }
