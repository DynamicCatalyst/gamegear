...
import com.gamegear.gamegear.model.*;
...
public class ProductService implements IProductService{
    ...
    private final com.gamegear.gamegear.repository.OrderItemRepository orderItemRepository;
    ...
    public Product updateProduct(ProductUpdateRequest request, Long productId) {
        return productRepository.findById(productId).map(existingProduct -> updateExistingProducts(existingProduct, request))
    ...
    @Override
    public PagedResponse<ProductDto> searchProducts(String category, List<String> brands,
                                                    String name, int page, int size) {
        // Normalize blank filters to null so the repository query ignores them.
        String categoryFilter = (category == null || category.isBlank() || category.equalsIgnoreCase("all"))
                ? null : category;
        List<String> brandFilter = (brands == null || brands.isEmpty()) ? null : brands;
        String nameFilter = (name == null || name.isBlank()) ? "" : name;

        Pageable pageable = PageRequest.of(Math.max(page, 0), Math.max(size, 1));
        Page<Product> result = productRepository.search(categoryFilter, brandFilter, nameFilter, pageable);
        List<ProductDto> dtos = getConvertedProducts(result.getContent());
    ...
    public List<Product> searchProductsByImage(MultipartFile image) throws IOException {
        List<Long> ListOfProductIds = chromaService.searchImageSimilarity(image);
        List<Product> products = productRepository.findAllById(ListOfProductIds);
        return products;
    }
